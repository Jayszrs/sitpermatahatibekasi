...
$academicYears=[]; $academicStart=(int)date('Y'); for($i=0;$i<=10;$i++){ $year=$academicStart+$i; $academicYears[$year.'/'.($year+1)]=$i===0?'Reguler':'Waiting List'; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'update_registration') {
            ...
            $level=$_POST['level']??''; $academicYear=$_POST['academic_year']??''; $admissionTrack=$_POST['admission_track']??'reguler'; $registrationStatus=$_POST['registration_status']??'baru'; $documentStatus=$_POST['document_status']??'belum_lengkap'; $paymentStatus=$_POST['payment_status']??'belum_bayar';
            ...
            if(!isset($academicYears[$academicYear]) || !in_array($admissionTrack,['reguler','waiting_list'],true)) throw new RuntimeException('Tahun ajaran atau jalur pendaftaran tidak valid.');
            ...
            $stmt=$pdo->prepare('UPDATE spmb_registrations SET registration_number=?,student_name=?,student_nik=?,gender=?,birth_place=?,birth_date=?,parent_name=?,parent_nik=?,family_card_number=?,whatsapp=?,level=?,academic_year=?,admission_track=?,previous_school=?,address=?,registration_status=?,document_status=?,payment_status=?,payment_notes=?,payment_updated_by=? WHERE id=?');
            $stmt->execute([trim($_POST['registration_number']??'')?:null,$studentName,trim($_POST['student_nik']??'')?:null,($_POST['gender']??'')?:null,trim($_POST['birth_place']??'')?:null,($_POST['birth_date']??'')?:null,$parentName,trim($_POST['parent_nik']??'')?:null,trim($_POST['family_card_number']??'')?:null,$whatsapp,$level,$academicYear,$admissionTrack,trim($_POST['previous_school']??'')?:null,trim($_POST['address']??'')?:null,$registrationStatus,$documentStatus,$paymentStatus,trim($_POST['payment_notes']??'')?:null,portal_user()['id'],$id]);
...
if(isset($academicYears[$yearFilter])) { $where[]='academic_year = ?'; $params[]=$yearFilter; }
...
if(($_GET['export']??'')==='csv') { header('Content-Type: text/csv; charset=UTF-8'); header('Content-Disposition: attachment; filename="rekap-spmb-'.date('Ymd-His').'.csv"'); $out=fopen('php://output','w'); fwrite($out,"\xEF\xBB\xBF"); fputcsv($out,['Nomor','Nama Siswa','NIK','Jenjang','Tahun Ajaran','Jalur','Orang Tua/Wali','WhatsApp','Status Pendaftaran','Status Dokumen','Status Pembayaran','Total Bayar','Tanggal Daftar']); foreach($registrations as $row) fputcsv($out,[$row['registration_number'],$row['student_name'],$row['student_nik'],$row['level'],$row['academic_year'],$row['admission_track'],$row['parent_name'],$row['whatsapp'],$row['registration_status'],$row['document_status'],$row['payment_status'],$row['payment_amount'],$row['created_at']]); fclose($out); exit; }
...
<section class="portal-panel" style="margin-bottom:22px">
    ...
    <form class="portal-form portal-form-grid" method="post">
        ...
        <div class="field"><label>Jenjang</label><select name="level" required><?php foreach($spmbLevels as $level=>$label): ?><option value="<?php echo esc($level); ?>" <?php echo $editPayment['level']===$level?'selected':''; ?>><?php echo esc($label); ?></option><?php endforeach; ?></select></div><div class="field"><label>Tahun ajaran</label><select name="academic_year" required><?php foreach($academicYears as $year=>$track): ?><option value="<?php echo esc($year); ?>" <?php echo $editPayment['academic_year']===$year?'selected':''; ?>><?php echo esc($year.' — '.$track); ?></option><?php endforeach; ?></select></div>
        <div class="field"><label>Jalur penerimaan</label><select name="admission_track"><option value="reguler" <?php echo $editPayment['admission_track']==='reguler'?'selected':''; ?>>Reguler</option><option value="waiting_list" <?php echo $editPayment['admission_track']==='waiting_list'?'selected':''; ?>>Waiting List</option></select></div>
...
<section class="portal-panel">
    <div class="panel-head"><h3>Data Pendaftar</h3><a class="portal-action secondary small" href="?<?php echo esc(http_build_query(['q'=>$search,'status'=>$statusFilter,'year'=>$yearFilter,'export'=>'csv'])); ?>">Unduh Rekap CSV</a></div>
    <form class="filters" method="get"><input name="q" value="<?php echo esc($search); ?>" placeholder="Cari nomor, NIK, siswa, orang tua, WA"><select name="year"><option value="">Semua tahun ajaran</option><?php foreach($academicYears as $year=>$track): ?><option value="<?php echo esc($year); ?>" <?php echo $yearFilter===$year?'selected':''; ?>><?php echo esc($year); ?></option><?php endforeach; ?></select><select name="status"><option value="">Semua status</option><option value="belum_bayar" <?php echo $statusFilter==='belum_bayar'?'selected':''; ?>>Belum bayar</option><option value="sebagian" <?php echo $statusFilter==='sebagian'?'selected':''; ?>>Sebagian</option><option value="lunas" <?php echo $statusFilter==='lunas'?'selected':''; ?>>Lunas</option></select><button class="portal-action small">Filter</button><a class="portal-action secondary small" href="<?php echo SITE_URL; ?>/portal/payments">Reset</a></form>
    <div class="portal-table-wrap"><table class="portal-table"><thead><tr><th>Calon Siswa</th><th class="hide-mobile">Jenjang / Tahun</th><th>Orang Tua / WA</th><th>Status</th><th>Nominal</th><th class="hide-mobile">Tanggal</th><th>Aksi</th></tr></thead><tbody>
    <?php if (!$registrations): ?><tr><td colspan="7" class="empty-state">Belum ada data pendaftar yang cocok.</td></tr><?php endif; ?>
    <?php foreach ($registrations as $item): ?><tr><td><strong><?php echo esc($item['student_name']); ?></strong><br><small style="color:var(--portal-muted)"><?php echo esc($item['registration_number'] ?: 'Belum ada nomor'); ?></small></td><td class="hide-mobile"><?php echo esc($item['level']); ?><br><small><?php echo esc($item['academic_year'].' · '.ucwords(str_replace('_',' ',$item['admission_track']))); ?></small></td><td><?php echo esc($item['parent_name']); ?><br><small><?php echo esc($item['whatsapp']); ?></small></td><td><span class="status <?php echo esc($item['payment_status']); ?>"><?php echo esc(ucwords(str_replace('_',' ',$item['payment_status']))); ?></span><br><small><?php echo esc(ucwords(str_replace('_',' ',$item['document_status']))); ?></small></td><td class="amount"><?php echo portal_rupiah($item['payment_amount']); ?></td><td class="hide-mobile"><?php echo $item['payment_date'] ? esc(date('d/m/Y', strtotime($item['payment_date']))) : '-'; ?></td><td><div class="table-actions"><a class="portal-action secondary small" href="<?php echo SITE_URL; ?>/portal/payments?edit=<?php echo (int)$item['id']; ?>">Data</a><a class="portal-action small" href="<?php echo SITE_URL; ?>/portal/payments?pay=<?php echo (int)$item['id']; ?>">Bayar</a></div></td></tr><?php endforeach; ?>
    </tbody></table></div>
</section>
<section class="portal-panel" style="margin-top:22px"><div class="panel-head"><h3>Riwayat Transaksi</h3></div><div class="portal-table-wrap"><table class="portal-table"><thead><tr><th>Kuitansi</th><th>Siswa</th><th class="hide-mobile">Jenis</th><th>Nominal</th><th class="hide-mobile">Metode</th><th class="hide-mobile">Tanggal</th><th class="hide-mobile">Petugas</th><th>Status/Aksi</th></tr></thead><tbody><?php if(!$transactions): ?><tr><td colspan="8" class="empty-state">Belum ada transaksi pembayaran.</td></tr><?php endif; ?><?php foreach($transactions as $trx): ?><tr><td><strong><?php echo esc($trx['receipt_number']); ?></strong><?php if($trx['reference_number']): ?><br><small>Ref: <?php echo esc($trx['reference_number']); ?></small><?php endif; ?></td><td><?php echo esc($trx['student_name']); ?><br><small><?php echo esc($trx['payer_name']?:'-'); ?></small></td><td class="hide-mobile"><?php echo esc($trx['payment_type']); ?></td><td class="amount"><?php echo portal_rupiah($trx['amount']); ?></td><td class="hide-mobile"><?php echo esc($trx['payment_method']); ?></td><td class="hide-mobile"><?php echo esc(date('d/m/Y',strtotime($trx['payment_date']))); ?></td><td class="hide-mobile"><?php echo esc($trx['recorder']?:'-'); ?></td><td><?php if($trx['status']==='verified'): ?><form method="post" onsubmit="return confirm('Batalkan transaksi ini?')"><input type="hidden" name="_token" value="<?php echo esc(portal_csrf_token()); ?>"><input type="hidden" name="action" value="cancel_payment"><input type="hidden" name="id" value="<?php echo (int)$trx['registration_id']; ?>"><input type="hidden" name="payment_id" value="<?php echo (int)$trx['id']; ?>"><button class="portal-action danger small">Batalkan</button></form><?php else: ?><span class="status inactive">Dibatalkan</span><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div></section>
