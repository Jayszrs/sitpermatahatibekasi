...
<section class="portal-panel" style="margin-bottom:22px">
    ...
    <div class="portal-table-wrap"><table class="portal-table"><thead><tr><th>Album</th><th class="hide-mobile">Slug</th><th>Foto</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
    <?php foreach ($galleryAlbums as $album): ?><tr><td><strong><?php echo esc($album['title']); ?></strong><br><small style="color:var(--portal-muted)"><?php echo esc($album['description'] ?: '-'); ?></small></td><td class="hide-mobile"><?php echo esc($album['slug']); ?></td><td><?php echo (int)$album['photo_count']; ?> foto</td><td><?php echo ((int)$album['is_active'] === 1) ? 'Aktif' : 'Nonaktif'; ?></td><td><div class="table-actions"><a class="portal-action secondary small" href="<?php echo SITE_URL; ?>/galeri-detail.php?id=<?php echo (int)$album['id']; ?>" target="_blank" rel="noopener">Lihat</a><form method="post" onsubmit="return confirm('Hapus album ini beserta semua fotonya?')"><input type="hidden" name="_token" value="<?php echo esc(portal_csrf_token()); ?>"><input type="hidden" name="action" value="delete_gallery_album"><input type="hidden" name="id" value="<?php echo (int)$album['id']; ?>"><button class="portal-action danger small">Hapus</button></form></div></td></tr><?php endforeach; ?>
    </tbody></table></div>
</section>

<section class="portal-panel">
    <div class="panel-head"><h3>Foto Galeri (<?php echo count($gallery); ?>)</h3></div>
    <div class="portal-table-wrap"><table class="portal-table"><thead><tr><th>Foto</th><th>Judul</th><th class="hide-mobile">Album</th><th>Unit/Publikasi</th><th class="hide-mobile">Keterangan</th><th>Aksi</th></tr></thead><tbody>
    <?php foreach ($gallery as $item): ?><tr><td><img src="<?php echo esc($item['image']); ?>" alt=""></td><td><strong><?php echo esc($item['title']); ?></strong></td><td class="hide-mobile"><?php echo esc($item['album_title']); ?></td><td><?php echo $item['unit_slug']?esc(strtoupper($item['unit_slug'])):'-'; ?><?php if($item['instagram_url']): ?><br><a href="<?php echo esc($item['instagram_url']); ?>" target="_blank" rel="noopener">Instagram &nearr;</a><?php endif; ?></td><td class="hide-mobile"><?php echo esc($item['description'] ?: '-'); ?></td><td><form method="post" onsubmit="return confirm('Hapus foto ini?')"><input type="hidden" name="_token" value="<?php echo esc(portal_csrf_token()); ?>"><input type="hidden" name="action" value="delete_gallery_photo"><input type="hidden" name="id" value="<?php echo (int)$item['id']; ?>"><button class="portal-action danger small">Hapus</button></form></td></tr><?php endforeach; ?>
    </tbody></table></div>
</section>
